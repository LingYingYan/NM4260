...
if (self.card_data != undefined) {
    ...
    if (self.reveal >= obj_player_state.max_vision) {
        ...
        scribble_text.scale(text_scale).draw(self.x, self.y - self.sprite_height / 4);
        text_y = self.y - self.sprite_height / 4 + scribble_text.get_height() + y_padding;
    } else {
        text_y = self.y - self.sprite_height / 4 + y_padding;
    }
