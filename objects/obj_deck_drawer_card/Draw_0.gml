...
if (instance_exists(obj_deck_drawer)) {
	if (obj_deck_drawer.can_remove) {
	    var glow_color = c_yellow;     
	    var glow_size = 6;     
	    var glow_alpha = 0.4;
	    ...
	    // draw the card multiple times around its center
	    for (var i = 0; i < 360; i += 45) {
	        var gx = self.x + lengthdir_x(glow_size, i);
	        var gy = self.y + lengthdir_y(glow_size, i);
	        draw_sprite_ext(self.sprite_index, self.image_index, gx, gy,
	            self.image_xscale, self.image_yscale, self.image_angle, glow_color, glow_alpha);
	    }

	    gpu_set_blendmode(bm_normal); // restore normal blend mode
	    draw_set_alpha(1);
	}
}
